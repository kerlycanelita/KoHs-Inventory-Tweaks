...
- Stopped merging a deliberate quick open-and-close into a single open. Two presses sharing one GLFW batch were read as a failing switch, and the whole argument for that rested on a batch being one rendered frame -- eight milliseconds at 120 fps, which no hand produces. At 30 fps a frame is thirty-three milliseconds, inside the range a hand does produce, so on a slow frame the player's second press was dropped. The merge now also requires the batch itself to be shorter than the quickest human double tap; above that floor both presses go to Vanilla untouched. The measurement is taken once per frame rather than in the key callback, because GLFW delivers a whole queued burst inside one `pollEvents` and two presses forty milliseconds apart still reach the callbacks microseconds apart.
- Stopped the double-press merge from eating a click it then handed back to Vanilla. The merge ran before the conflict checks, so a batch carrying two inventory presses that also shared its key with another mapping had one of the two queued clicks consumed and was then declared a Vanilla fallback -- against the rule the controller states for itself, that a fallback leaves the complete Vanilla queue untouched. The visible outcome happened to survive, because Vanilla drains that queue in a `while` loop that opens a fresh inventory per click and ends open either way, but the reported decision no longer matched what had been done to the queue. Every check now runs before anything is consumed, and the surplus click is dropped only once the opening is committed.
- Named every reason a press waited for the client tick. The readout under Super Fast Inventory spoke only for the two conflict reasons and drew nothing at all for the rest, so the most frequent one in a fight -- a mouse button still held, which 1.0.8 deliberately yields on -- looked exactly like the feature not working. A held button, an inactive window, several queued presses and an already-taken click each say so now, and anything else falls back to one generic line.
- Applied both to the maintained five-version set. 1.21.10 receives the controller fix; it has no readout to correct.

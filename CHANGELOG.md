...
- Moved the early inventory opening from the GLFW poll to the moment Minecraft finishes draining the tasks that poll queued. Every input callback is posted through `Minecraft#execute` and only runs inside `runAllTasks`, so deciding at the poll read input that was a frame old, left cursor samples captured while the mouse was still grabbed queued behind the opening, and ran before the frame's packets were processed. The samples were applied after the screen opened and overwrote the pointer this mod had just placed, so for one frame the system cursor sat on the landing point while the game resolved the hovered slot somewhere else. An offhand key pressed right after the inventory key then acted on the wrong slot, or on none. The decision still runs ahead of `Minecraft#tick`, which is what makes the feature fast.

	...
	private void addMacroButtons(final int top, final int height, final int gap, final boolean compact) {
		...
		if (!compact) {
			...
			Button lab = this.addRenderableWidget(button("kohs_inventory_debug.screen.stress_lab",
				this.contentX + 5 * (width + gap), top, width, height, ignored -> this.minecraft.gui.setScreen(new StressLabScreen(this))));
			lab.active = active;
			return;
		}
		...
		Button lab = this.addRenderableWidget(button("kohs_inventory_debug.screen.stress_lab",
			...
			ignored -> this.minecraft.gui.setScreen(new StressLabScreen(this))));
		lab.active = active;
	}

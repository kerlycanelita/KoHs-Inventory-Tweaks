...
public final class ColorPaletteWidget extends AbstractWidget {
	...
	protected void renderWidget(
		...
		for (int row = 0; row < ROWS; row++) {
			...
			for (int column = 0; column < COLUMNS; column++) {
				...
				if (swatch == selected) {
					graphics.renderOutline(x0, y0, Math.max(1, x1 - x0), Math.max(1, y1 - y0), 0xFFFFFFFF);
				}
			}
		}
	...
	public void onClick(final MouseButtonEvent event, final boolean doubleClick) {
		...
		int column = Math.min(COLUMNS - 1, (int) ((event.x() - gridX) * COLUMNS / gridWidth));
		int row = Math.min(ROWS - 1, (int) ((event.y() - gridY) * ROWS / gridHeight));

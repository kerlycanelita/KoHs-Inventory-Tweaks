...
public final class AccessibilityRenderController {
	...
	public static void drawFocusedSlot(
		...
		graphics.outline(slot.x - 2, slot.y - 2, 20, 20, color);
		graphics.outline(slot.x - 1, slot.y - 1, 18, 18, UiRender.withAlpha(0xFFFFFF, Math.min(180, alpha)));
	}
	...
	public static void drawPreviewFocus(
		...
		graphics.outline(x - 1, y - 1, 18, 18, UiRender.withAlpha(0xFFFFFF, Math.min(180, alpha)));
	}
}

...
public final class CursorLandingController {
	...
	private static @Nullable Screen verificationScreen;
	...
	public static void onClientTick(final Minecraft minecraft) {
		...
		verificationPending = false;
		if (minecraft == null || minecraft.gui.screen() != verificationScreen || System.nanoTime() > centerGuardUntil) {
			return;
		}

		CursorTarget target = classify(minecraft.gui.screen());
		if (target != null && shouldPlaceCursor(target)) {
			warp(minecraft, resolvePhysicalPosition(minecraft, minecraft.gui.screen(), target, true));
		}
	}
	...
	public static boolean recoverUnexpectedCenterEvent(final Minecraft minecraft, final double x, final double y) {
		...
		double[] desired = resolvePhysicalPosition(minecraft, minecraft.gui.screen(), target, true);

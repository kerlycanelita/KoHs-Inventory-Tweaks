...
public final class ItemHighlighterController {
	...
	public static void drawHighlightLayer(
		...
		if (foreground) {
			graphics.outline(x, y, size, size, 0xFF000000 | highlight.borderColor & 0xFFFFFF);
			graphics.outline(x + 1, y + 1, Math.max(1, size - 2), Math.max(1, size - 2), 0x72000000 | highlight.borderColor & 0xFFFFFF);
		} else {
